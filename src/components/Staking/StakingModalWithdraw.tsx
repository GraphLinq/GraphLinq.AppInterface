...
export const StakingModalWithdraw: React.FC<StakingModalWithdrawProps> = (props: any) => {
  ...
  async function doWithdraw() {
    ...
    if (props.withdrawAmount <= 0) {
      setError(
        `Invalid amount to withdraw from the staking contract: ${props.withdrawAmount} GLQ`
      );
      return;
    }
      ...
      >
        ...
        <ModalContent className="mod mod-cre">
              ...
              >
                ...
                <p>
                  ...
                  <small>
                    Transaction hash :{" "}
                    <a
                      href={`https://etherscan.com/tx/${success}`}
                      target="_blank"
                    >
                      {success}
                    </a>
          ...
          <ModalFooter className="fot">
            <Button
              className="bt"
              style={{ padding: 15, fontSize: "15px" }}
              onClick={doWithdraw}
            >
              Yes, I'm sure
            </Button>
            <Button
              colorScheme="base"
              style={{ padding: 15, fontSize: "15px", fontWeight: 500 }}
              onClick={onClose}
            >
              No, cancel
            </Button>
