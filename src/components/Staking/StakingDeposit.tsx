...
export const StakingDeposit: React.FC<StakingDepositProps> = (props: any) => {
  ...
  const tokenContract = useTokenContract(
    process.env.REACT_APP_GRAPHLINQ_TOKEN_CONTRACT
  );
  ...
  async function doStake(e: any) {
    ...
    if (asNumber <= 0) {
      setError(
        `Invalid amount to deposit on the balance contract: ${asNumber} GLQ`
      );
      toast({
        position: "bottom-right",
        render: () => (
          <ToastError
            description={`Invalid amount to deposit on the balance contract: ${asNumber} GLQ`}
          />
        ),
      });
      return;
    ...
    try {
      ...
      const currentBalanceDecimal: any = utils.parseEther(
      ...
      if (parseFloat(decimalAmount) > parseFloat(currentBalanceDecimal)) {
        ...
        toast({
          ...
          render: () => (
            <ToastError
              description={`You only have ${balance.amount} GLQ in your wallet.`}
            />
          ),
        });
        return;
      ...
      toast({
        ...
        render: () => (
          <ToastSuccess
            title="Deposit successfully completed !"
